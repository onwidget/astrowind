...
title: "Algolia HN Search Complaints and Trieve's Discovery Quality"
excerpt: Comparing the search quality between Algolia and Trieve with HN search complaints.
...
Algolia provides only three results, from 9-12 years ago. Trieve provides dozens of results to help the searcher along their way.
<ImageComparison
...
Having one extra word in the search causes it to fail in Trieve&#x27;s fulltext search (see the search with or without &quot;new&quot;). The issue does not seem to be an Algolia query limit (512 chars).
<ImageComparison
...
Algolia&#x27;s result appear to match on <code>video</code>, not only <code>[video]</code>. Trieve&#x27;s hybrid and semantic search types do very well on the exact match, but not perfect. Trieve does better if in quotes, like [&quot;[video]&quot;], which does not seem to have an affect here in Algolia.
<ImageComparison
