...
1. We have enough data to show the most linked top-level domains and plan to make a site for displaying that list.
2. Our recommendation system supports both positive and negative examples (see our [steam games recommendation system here](https://steamdb.trieve.ai/) to see that in action) and we would like to make an interface for that on HN. More advanced recommendations seem useful for "past" links in particular.
3. Come back and add LTR
